...
export async function POST(req: NextRequest) {
  try {
    const { invoiceId, payerEmail, accessToken } = await req.json();

    if (!invoiceId || !payerEmail) {
      return NextResponse.json({ error: 'invoiceId and payerEmail are required' }, { status: 400 });
    }
    ...
    // Import starkzap server-side only
    const { StarkZap, OnboardStrategy, accountPresets, Amount, mainnetTokens, sepoliaTokens, fromAddress } = await import('starkzap');
    ...
    const { wallet } = await sdk.onboard({
      ...
      privy: {
        resolve: async () => {
          const walletRes = await fetch(`${process.env.NEXT_PUBLIC_URL || 'http://localhost:3000'}/api/wallet/starknet`, {
            method: 'POST',
            headers: {
          ...
          return {
            ...
            serverUrl: `${process.env.NEXT_PUBLIC_URL || 'http://localhost:3000'}/api/wallet/sign`,
          };
        },
      },
      deploy: 'if_needed',
